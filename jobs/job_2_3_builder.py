...
for size in graph_sizes:
    ...
    with open(path.join(job_name, name), mode="w", encoding="utf-8") as job_file:
        ...
        job_file.write(f"\n")
        job_file.write(f"python3 ../2_3_category_production_ngram_tsa.py \\\n")
        job_file.write(f"           -b {int(size/2)} \\\n")
        job_file.write(f"           -c bbc \\\n")
        job_file.write(f"           -f 0.3 \\\n")
        job_file.write(f"           -i 0.05 \\\n")
        job_file.write(f"           -d cosine \\\n")
        job_file.write(f"           -l 10 \\\n")
        job_file.write(f"           -m log_ngram \\\n")
        job_file.write(f"           -n 0.99 \\\n")
        job_file.write(f"           -r 5 \\\n")
        job_file.write(f"           -s 15 \\\n")
